...
void evolve_hom_constrained(int n_x, int n_y, double delta_x, double delta_y, 
double kappa, double A, double delta_t, int time_steps, double t0,
fftw_complex *comp, double Ceff[3][3][3][3], double **sigma_T, 
double **epsilon_T, double ave_comp, double **sig_app, int STEPS, 
...
/* The acoustic tenosr Omega and the (homogeneous) base solution */
...
for(i2=0; i2 < n_y; ++i2){
	...
	strain[1][1] = 
	E[1][1] + creal(eps_star11[J]) - epsilon_T[1][1]*b;
	strain[1][2] = 
