...
mu = 400.0; /*  The shear modulus of the matrix phase */
lambda = 600.0; /* The other Lame's constant for the matrix phase */
...
Omega12 = 0.03; /* The 12 component of the eigenstrain */
...
aplusb = a + b;
	...
	for(j=0; j < 22; ++j){
		...
		if( (X*X/(a*a) + Y*Y/(b*b)) <= 1.0 ){
			/* Principal eigenstrain part: Equations 14 and 15 */
			/* Note: sigxx, sigyy and sigxy are, respectively, */
			/* P11, P22 and P12 of Equation 14 */
			/* chi and eta in Equation 15 are referred to */
			...
			/* Shear eigenstrain part */
			sigxxs = -8*mu*epsilon12*a*b*sin(2.0*theta)/((kappa+1.0)*(aplusb)*(aplusb));
			sigyys = -sigxxs;
			sigxys = -8*mu*epsilon12*a*b*cos(2.0*theta)/((kappa+1.0)*(aplusb)*(aplusb));
			...
			/* Note: sigxx, sigyy and sigxy are, respectively, */
			/* p11, p22 and p12 of Equation 6 */
			/* chi and eta in Equation 13 are referred to */
			/* as 2 and 1 respectively */
